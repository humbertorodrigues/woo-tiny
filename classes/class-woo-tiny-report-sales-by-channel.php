...
class WC_Report_Woo_Tiny_Sales_By_Channel extends WC_Admin_Report
{
    ...
    {
        if ( isset( $_GET['channel_ids'] ) && is_array( $_GET['channel_ids'] ) ) {
            $this->channel_ids = array_filter( array_map( 'absint', $_GET['channel_ids'] ) );
        } elseif ( isset( $_GET['channel_ids'] ) ) {
            $this->channel_ids = array_filter( array( absint( $_GET['channel_ids'] ) ) );
        }
    }
    ...
    public function get_chart_widgets()
    {
        ...
        $widgets[] = array(
            'title' => '',
            'callback' => array($this, 'channels_widget'),
        );
    ...
    {
        $this->channel_ids_titles = [];
        foreach ($this->channel_ids as $channel_id){
            $channel = get_post($channel_id);
            if ($channel) {
                $this->channel_ids_titles[] = $channel->post_title;
            } else {
                $this->channel_ids_titles[] = '#' . $channel_id;
            }
        }

        echo '<p><strong>' . implode(', ', $this->channel_ids_titles) . '</strong></p>';
        echo '<p><a class="button" href="' . esc_url(remove_query_arg('channel_ids')) . '">' . esc_html__('Reset', 'woocommerce') . '</a></p>';
    }

    /**
     * Output sellers widget.
     */
    public function channels_widget()
    {
        $channels = get_posts([
        ...
        ?>
        <h4 class="section_title open"><span>Pesquisar canais</span></h4>
        <div class="section">
            <form method="GET">
                <div>
    ...
    public function get_main_chart()
    {
        ...
        if (empty($this->channel_ids)) {
            ...
            $query = [
                'debug' => false,
                'group_by' => 'bw_canal_venda,' . $this->group_by_query,
